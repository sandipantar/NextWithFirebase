...
import { CatagoryContext } from './components/catagory/CatagoryContext';
import CatagoryItem from './components/catagory/catagoryItem';
...
export default function Catagory(){
    ...
    useEffect(() => {
      ...
      const catData = onSnapshot(qry, (QuerySnapshot) => {
        setFetch(QuerySnapshot.docs.map(doc => ({...doc.data(), id: doc.id, createdAt: doc.data().
...
// }, 5000);
    return(
      <CatagoryContext.Provider value={{fetch, setFetch}}>
        <div>
          <Head>
            ...
            <div>
              {/* <pre>{JSON.stringify(fetch,null,'\t')}</pre> */}
              ...
              {Array.from(fetch).map(catDet => <CatagoryItem key={catDet.id}
              id={catDet.id}
              catagory={catDet.catagory}
              catagoryFlag={catDet.catagoryFlag}
              createdAt={catDet.createdAt}
              />)}
              {/* <div
                      open={open}
                      onClick={handleClose} 
